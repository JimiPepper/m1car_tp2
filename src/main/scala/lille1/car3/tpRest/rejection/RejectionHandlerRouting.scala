...
trait RejectionHandlerRouting
{
	...
	**/
    ...
    case ValidationRejection(message, cause) :: _ =>
      complete(HttpResponse(status = 403, entity = HttpEntity(`text/html`, "<p>Votre session a expire, veuillez <a href=\"http://localhost:8080\">vous reconnectez</a>")))
